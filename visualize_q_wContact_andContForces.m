function visualize_q_wContact_andContForces(q,contacts,contactForces,forceScaler,L_0,figHandle,color,style)
    ...
    for i=1:length(contacts)
        ...
        line([xc_c(contacts(i)),xc_c(contacts(i))+forceScaler.*contactForces(i).*sin_phi_s(contacts(i).*q(4))],[yc_c(contacts(i)),yc_c(contacts(i))-forceScaler.*contactForces(i).*cos_phi_s(contacts(i).*q(4))]); % plot the contact force
